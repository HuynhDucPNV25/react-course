    ...
    const PRODUCTS = [
        ...
        SHOPPING_CART.forEach(cart => {
            PRODUCTS.forEach(item=>{
                if (cart.id===item.id) {
                    ...
                    result+=item.price*cart.quantity
                    // console.log(result);
                }
            })
      ...
      function removeProductFromCart(productId) {
        // Write your code here
        const prod = SHOPPING_CART.find((item) => item.id === productId);
        const index = SHOPPING_CART.indexOf(prod);
        // console.log(index, "check vị trí");
        // console.log(prod);
        if (prod.quantity >= 2) {
            prod.quantity -= 1
        } else {
